...
from greenbutton_objects import enums, objects, utils
...
class ReadingType(Resource):
    def __init__(self, entry, meterReadings=[]):
        ...
        self.commodity = utils.getEntity(obj, "espi:commodity", lambda e: enums.CommodityType(int(e.text)))
        self.consumptionTier = utils.getEntity(
            obj,
            "espi:consumptionTier",
            lambda e: enums.ConsumptionTierType(int(e.text)),
        )
        self.currency = utils.getEntity(obj, "espi:currency", lambda e: enums.CurrencyCode(int(e.text)))
        ...
        self.powerOfTenMultiplier = utils.getEntity(obj, "espi:powerOfTenMultiplier", lambda e: int(e.text))
...
class IntervalBlock(Resource):
    def __init__(self, entry, meterReadings=[]):
        ...
        self.intervalReadings = sorted([
            objects.IntervalReading(ir, self) for ir in obj.findall("espi:IntervalReading", utils.ns)
        ])
